...
const filters = {
    bassboost: 'bass=g=[dB],dynaudnorm=f=200',
...
class Player {
     ...
     * });
     */
    setFilters (guildID, newFilters, amount) {
        return new Promise((resolve, reject) => {
            // Get guild queue
            const queue = this.queues.find((g) => g.guildID === guildID)
            if (!queue) return reject(new Error('Not playing'))

            if(newFilters.bassboost) this.bassBoost = amount
        ...
        return new Promise((resolve) => {
            ...
            Object.keys(queue.filters).forEach((filterName) => {
                if (queue.filters[filterName]) {
                    encoderArgsFilters.push(filterName == "bassboost"? filters[filterName].replace(/\[dB\]/g, this.bassBoost) : filters[filterName])
                }
            })
            let encoderArgs
